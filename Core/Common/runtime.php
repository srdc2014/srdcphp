...
function build_runtime_cache($append='') {
	...
	if(defined('SR_KEY')){ $content = preg_replace('/defined\(\'SR_KEY\'\) or define\(\'SR_KEY\',\'(.+?)\'\)\;/','',$content);exit;}
	...
	if(is_file(__ROOT__.$domain.'.php')){	
		...
		$content .=  sha1( $domain.$key['key'])==$key['code'] ? ' defined(\'SR_KEY\') ?  exit : define(\'SR_KEY\',true);' : 'define(\'SR_KEY\',false);';
	}else{
		$content .= 'define(\'SR_KEY\',false);';
	}
