...
include_once (SITEDIR."/Srdcphp/Common/common.php");
...
case '1':
	...
	if(strtolower($dir[1])=='srdcphp'){ echo '程序不能安装在srdcphp目录下，请更改为其他目录！';exit;} 
...
case '4':
	...
	if(intval($_GET['install'])){
			...
			$dbPrefix = empty($_POST['dbPrefix']) ? 'srdcphp_' : trim($_POST['dbPrefix']);
			...
			*/
			...
			$sqldata =   file_get_contents(SITEDIR.'Install/srdcphp_data.sql');
			sql_execute($sqldata, $dbPrefix);
			
			$sqldata = file_get_contents(SITEDIR.'Install/srdcphp_area.sql');
			sql_execute($sqldata, $dbPrefix);
			...
			if( $_POST['lang']){
				$langsql = file_get_contents(SITEDIR.'Install/srdcphp_lang.sql');
